...
func SendPatchCmd(deps *Dependencies, msg tui.SendPatchMsg) tea.Cmd {
	return func() tea.Msg {
		...
		if err != nil {
			return tui.PatchGeneratedMsg{Err: err}
		}
		return tui.PatchGeneratedMsg{RawPatch: rawPatch}
	}
}
...
func SendRawPatchCmd(deps *Dependencies, rawPatch []byte) tea.Cmd {
	return func() tea.Msg {
		...
		if account.ServiceProvider != "gmail" {
			if err := fetcher.AppendToSentMailbox(account, rawPatch); err != nil {
				log.Printf("Failed to append sent patch to Sent folder: %v", err)
			}
		}
